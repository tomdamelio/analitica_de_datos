...
7. **Badges vía espejo público de notebooks** *(agregado post-checkpoint, decisión de la cátedra)*. Colab solo abre notebooks de repos públicos y este repo es privado: los badges apuntan a `tomdamelio/analitica-de-datos-notebooks` (público), que el job `mirror-notebooks` del workflow sincroniza en cada push (solo `.ipynb`, misma ruta; nunca `data/` ni `context/`). Setup único requerido: crear ese repo público + secret `NOTEBOOKS_MIRROR_TOKEN` (ver README §Publicación).
