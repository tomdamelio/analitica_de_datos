"""
Piezas compartidas por TODOS los bloques del deck HTML (chrome + helpers de contenido).

Este archivo es el "starter kit": copialo a la carpeta del deck y ADAPTA lo marcado con
"AJUSTAR:" (paleta, logo, secciones, URL de notebooks) a la identidad del proyecto en cuestion.
Todo lo demas (CSS de layout, JS de navegacion) ya esta calibrado y no deberia hacer falta tocarlo.

Cada bloque tematico vive en su propio `contenido_bloqueN.py` y expone una lista `SLIDES` de
fragmentos HTML, armados con los helpers de aca (`slide`, `caja`, `bullets`, etc.). `build_todo.py`
los junta todos en el archivo final; cada `contenido_bloqueN.py` tambien se puede correr solo
(`python contenido_bloqueN.py`) para previsualizar nada mas que ese bloque mientras se itera.
"""

import json
from pathlib import Path

# AJUSTAR: si el deck tiene formulas, generar con scripts/embed_libs.py y descomentar.
...
# ---------------------------------------------------------------- paleta ----
# AJUSTAR: paleta institucional. Este es el default de referencia (probado en
# un curso real) — cambiar los hex si el proyecto tiene su propia identidad.
...
GRIS_CLARO = "#969696" # pie de pagina, lineas divisorias
TINTA = "#122535"      # texto principal

# AJUSTAR: un color por bloque tematico (hasta donde tenga la clase).
COLOR_BLOQUE = {1: AZUL, 2: NARANJA, 3: VERDE, 4: VIOLETA, 5: ROJO, 6: MARRON}

# AJUSTAR (opcional): si el deck tiene puntero a notebooks de practica (patron de curso).
NOTEBOOK_URL = "https://colab.research.google.com/github/<owner>/<repo>/blob/main/<ruta>.ipynb"

# AJUSTAR: dos variantes del logo institucional, en base64 (o dejar "" para no mostrar).
# Generar con:
#   python -c "import base64; print('data:image/png;base64,' + base64.b64encode(open('logo.png','rb').read()).decode())"
# LOGO_B64: chico, va arriba a la derecha en TODAS las slides de contenido (via slide()) —
#   version translucida/gris suele verse mejor ahi que a todo color (queda como marca de
#   agua discreta, no compite con el titulo).
# LOGO_COLOR_B64: grande, a todo color, SOLO en la portada (la slide 1 no usa slide(), se
#   arma a mano — ver el ejemplo de apertura/cierre en assets/ejemplo_apertura_cierre.py).
# Si a un deck le importa la marca institucional, asegurarse de setear LAS DOS variantes,
# no solo una — es un error comun no notar que son dos logos distintos, no el mismo chico
# reusado en dos tamanos.
LOGO_B64 = ""
LOGO_COLOR_B64 = ""

# AJUSTAR: mapa global de secciones (limites de TODA la clase, no solo un bloque) para
# el pie de pagina con progreso. `inicio`/`fin` son numeros de slide GLOBALES (1-indexed,
# contando toda la clase de punta a punta); `nombre: None` = sin pie (tipico de la apertura).
SECCIONES = [
    {"inicio": 1, "fin": 3, "nombre": None},
    {"inicio": 4, "fin": 10, "nombre": "Bloque 1"},
    {"inicio": 11, "fin": 20, "nombre": "Bloque 2"},
    # ... agregar un rango por bloque, terminando en el cierre.
]
...
def slide(numero, bloque, titulo, contenido_html, con_logo=True, extra_clase="",
    ...
    (contenido) + pie. `con_logo=False` tipicamente solo en la portada y en el
    cierre. `contenido_clase` agrega clases al <main>: "centrado" (todo
    centrado, para widgets o poco texto), "centrado-img" (una figura grande),
    "densa" (achica fuente/espaciado para slides con mucho contenido).
    """
    color = _color_bloque(bloque)
    titulo_html = f'<h1 class="titulo" style="color:{color}">{titulo}</h1>' if titulo else ""
    logo_html = f'<img class="logo" src="{LOGO_B64}" alt="logo">' if con_logo and LOGO_B64 else ""
...
def ecuacion(formula):
    """Ecuacion ESTATICA centrada (modo display, delimitadores $$...$$). Para una
    formula que se revela termino a termino, usar widget_formula_pasos en su lugar."""
    return f'<p class="ecuacion">$${formula}$$</p>'


def tabla(encabezados, filas):
    """
    `encabezados`: lista de strings. `filas`: lista de (titulo_fila, [valores]).
    """
    ...
    return f'<table class="tabla-datos"><thead><tr>{th}</tr></thead><tbody>{trs}</tbody></table>'
...
def colab_linea(texto_previo="Para la práctica vamos a"):
    return (f'<p class="colab-linea">{texto_previo}'
           f'<a class="colab-link" href="{NOTEBOOK_URL}" target="_blank" rel="noopener">'
           f'<strong>la notebook</strong></a>.</p>')


def slide_notebook(numero, bloque, encabezado, texto1="", texto2=""):
    """La slide "a la notebook" (bumper de puntero a la practica). El encabezado
    es un link real a NOTEBOOK_URL."""
    partes = [f'<a class="nb-link" href="{NOTEBOOK_URL}" target="_blank" rel="noopener">'
...
/* margin-left:auto (no solo justify-content en el padre) para que el logo
   ...
   header (p.ej. la portada de bloque no tiene <h1 class="titulo">, y sin esto
...
.tabla-datos {{ border-collapse:collapse; margin:.3em auto 0; font-size:.85em; }}
.tabla-datos th, .tabla-datos td {{ padding:.35em .7em; text-align:center; }}
.tabla-datos thead th {{ background:{NAVY}; color:white; font-weight:700; }}
.tabla-datos tbody th {{ text-align:left; font-weight:700; color:{TINTA}; }}
.tabla-datos tbody tr:nth-child(odd) {{ background:rgba(0,0,0,.03); }}

.columnas {{ display:flex; gap:1.2em; align-items:flex-start; justify-content:center; }}
.columnas .caja {{ flex:1; max-width:48% !important; }}

/* pie de pagina: circulos agrupados por seccion, CENTRADO en la slide. */
...
def armar_html(titulo_pagina, slides_html_list, extra_css="", extra_js=""):
    ...
    que estar cargada antes del `<div id="deck">`, no despues. KaTeX, al reves,
    va DESPUES de las slides (es pesado y no hace falta para el primer paint) —
    ver references/arquitectura.md si un widget necesita `katex.render()` directo.
    """
...
def escribir(nombre_archivo, slides_html_list, titulo_pagina=None, extra_css="", extra_js=""):
    titulo_pagina = titulo_pagina or f"Deck — {nombre_archivo}"
